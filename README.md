...
Rlimite requires `{torch}` with its LibTorch runtime, `{safetensors}`,
`{hfhub}`, and `{tok}`. A CUDA-enabled LibTorch installation is
recommended, but all examples below are evaluated on CPU. Each published
BF16 checkpoint is about 2.08 GB before runtime activations and caches.
