...
CPU under [`Rprof()`](https://rdrr.io/r/utils/Rprof.html) puts only
about a quarter of the time inside `torch_linear`; the largest remaining
share is R’s own method dispatch, `NextMethod`, `[[.nn_Module`, `$.R7`
and `find_method`, walking module attributes once per layer per token,
forty-eight layers deep. Casting weights to `float32` makes it slower
rather than faster, so the dtype is not the lever either.
